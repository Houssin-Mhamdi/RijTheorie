...
- **Stripe security (idempotency + auth + gating)**: `create-checkout` now authenticates the caller (createServerClient + cookies) and rejects if `userId !== user.id`; rate-limited 10/min per user; uses Stripe idempotency key `checkout_session_${user.id}_${planId}`. `connect` requires admin via `is_admin()`; `connect/status` requires any authenticated user. Webhook verifies signature, dedups via new `stripe_events` table (event.id) + `activate_subscription_from_payment` RPC (SECURITY DEFINER, service_role), which deactivates old subs, inserts new sub with `checkout_session_id`/`stripe_event_id`, and payouts dedup on `checkout_session_id`. Attempt reset is AUTOMATIC: `get_exam_attempt_status`/`can_attempt_exam` count attempts from MAX(user_subscriptions.start_date), so a new purchase resets attempts to the plan default — no manual reset needed. Student-view gating now always shows paid exams; a locked exam card (0 attempts) grays out + shows a "Buy/Reactiveren" button that expands an inline `#payment-section` with plan cards (no more redirect to admin `/subscriptions`). Exam detail page's blocked screen also returns to `/exams` instead of `/subscriptions`. Dead `/api/stripe/verify` route removed. New file `supabase-stripe-idempotency.sql` (must be run once in the DB). `.env.example` documents `STRIPE_SECRET_KEY`. Note: in-memory rate limiter is per-instance; for multi-region serverless use an external store.
- Stripe pitfalls to remember: never trust client price (always recompute from `subscription_plans.price`), verify webhook signature, keep secrets server-only, store event.id to make retries idempotent, let Stripe own entitlement while the DB mirrors it.
