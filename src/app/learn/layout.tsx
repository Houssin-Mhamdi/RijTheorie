        ...
        <header className="border-b border-outline-variant/20 bg-surface">
          <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => setLogoutOpen(true)} className="text-label-sm text-on-surface-variant hover:text-error transition-colors">Uitloggen</button>
            </div>
          </div>
        </header>
      ...
      <Dialog open={logoutOpen} onOpenChange={setLogoutOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={async () => { await supabase.auth.signOut(); router.push("/") }} className="flex items-center gap-2">
              <LogOut size={16} />
              Uitloggen
            </Button>
          </DialogFooter>
