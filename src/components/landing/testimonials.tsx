      ...
      <div className="max-w-container-max-width mx-auto px-margin-desktop">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map((testimonial) => (
            <div key={testimonial.name} className="bg-white p-8 rounded-2xl shadow-sm border border-surface-container-highest">
              <div className="flex items-center gap-2 mb-6">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className="text-secondary-container fill-current" size={20} />
                ))}
              </div>
              <p className="italic text-body-md text-on-surface-variant mb-8">&ldquo;{t(testimonial.quoteKey)}&rdquo;</p>
              <div className="flex items-center gap-4">
                <img className="size-12 rounded-full border-2 border-secondary-container" src={testimonial.avatar} alt={testimonial.name} />
                <div>
                  <div className="font-semibold text-label-md text-primary">{testimonial.name}</div>
                  <div className="font-medium text-label-sm text-on-surface-variant">{testimonial.role}</div>
                </div>
              </div>
            </div>
          ))}
